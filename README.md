...
<!-- TODO: add section describing the difference between proxying a request versus redirecting the request, need to figure out how to create an example where the origin can be maintained such that it's not a cross-origin redirect -->
...
<!-- TODO: could properly validate this by having separate CORS configuration for the proxy versus the initial example to show that CORS middleware runs twice even if proxied -->
