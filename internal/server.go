...
func getAddress(protocol, address, port string) string {
	address = fmt.Sprintf("%s://%s", protocol, address)
	if configProxyPort != "" {
		address += ":" + configProxyPort
	}
	return address
}
...
func endpointRedirect(writer http.ResponseWriter, request *http.Request) {
	var uri string

	redirectAddress := getAddress("http", configRedirectAddress, configRedirectPort)
	endpoint := request.URL.Query().Get("endpoint")
	endpoint = strings.ReplaceAll(endpoint, "/", "")
	switch endpoint {
	default:
		uri = fmt.Sprintf("%s/%s", redirectAddress, endpoint)
	case "":
		uri = redirectAddress
	}
...
func endpointProxy(writer http.ResponseWriter, request *http.Request) {
	...
	proxyAddress := getAddress("http", configProxyAddress, configProxyPort)
	endpoint := request.URL.Query().Get("endpoint")
	endpoint = strings.ReplaceAll(endpoint, "/", "")
	switch endpoint {
	default:
		uri = fmt.Sprintf("%s/%s", proxyAddress, endpoint)
	case "":
		uri = proxyAddress
	}
...
func launchServer(wg *sync.WaitGroup, chErr chan error) *http.Server {
	...
	go func() {
		...
		if configProxyAddress != "" && configProxyPort != "" {
			...
			router.HandleFunc("/redirect", endpointRedirect).Methods(http.MethodPost, http.MethodGet, http.MethodOptions)
		}
